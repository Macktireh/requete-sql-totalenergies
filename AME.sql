...
/** Table KNVV => KNVV **/
FROM SAPC47.KNVV@P2R.WORLD KNVV,

/** Table KNVV - Replace the country => T2 **/
(SELECT ZZPRCTR, max(ERDAT) lastdate FROM SAPC47.KNVV@P2R.WORLD
WHERE substr(ZZPRCTR,1,2) in ('BR','MQ','DO','GP','JM','MX','PR','VI')
--WHERE substr(ZZPRCTR,1,2) in ('MU')
AND VTWEG LIKE '20' GROUP BY ZZPRCTR) T2,

/** Table CEPC => CEPC **/
(SELECT PRCTR, KHINR, DATAB , MAX(DATBI) AS DATBI FROM SAPC47.CEPC@P2R.WORLD WHERE SUBSTR(PRCTR,6,1) = 'S' 
GROUP BY PRCTR, KHINR, DATAB) CEPC,

/** Table KNA1 => KNA1 **/
SAPC47.KNA1@P2R.WORLD KNA1,

/** Table KNA1 => T6 **/
(SELECT KUNNR, MAX(ERDAT) AS lastdate2
    FROM SAPC47.KNA1@P2R.WORLD
    GROUP BY KUNNR) T6,


/** Table CEPCT => CEPCT **/
SAPC47.CEPCT@P2R.WORLD CEPCT
...
AND KNVV.VKORG in ('0289','0817','0177','0818','0931','0492','0941','ZUVI')
