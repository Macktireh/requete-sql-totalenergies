...
/** Table KNVV => KNVV **/
FROM SAPC47.KNVV@P2K.WORLD KNVV,

/** Table KNVV - Replace the country => T2 **/
(SELECT ZZPRCTR, max(ERDAT) lastdate FROM SAPC47.KNVV@P2K.WORLD
WHERE substr(ZZPRCTR,1,2) in ('AO','KE','TZ','UG','ZW','NG','ZA','NA','BW','GH','MW','MU','MZ','ZM','ER','SZ')
--WHERE substr(ZZPRCTR,1,2) in ('MU')
AND VTWEG LIKE '20' GROUP BY ZZPRCTR) T2,

/** Table CEPC => CEPC **/
(SELECT PRCTR, KHINR, DATAB , MAX(DATBI) AS DATBI FROM SAPC47.CEPC@P2K.WORLD WHERE SUBSTR(PRCTR,6,1) = 'S' 
GROUP BY PRCTR, KHINR, DATAB) CEPC,

/** Table KNA1 => KNA1 **/
SAPC47.KNA1@P2K.WORLD KNA1,

/** Table KNA1 => T6 **/
(SELECT KUNNR, MAX(ERDAT) AS lastdate2
    FROM SAPC47.KNA1@P2K.WORLD
    GROUP BY KUNNR) T6,


/** Table CEPCT => CEPCT **/
SAPC47.CEPCT@P2K.WORLD CEPCT
...
AND KNVV.VKORG in ('0192','0197','0810','0815','0816','0830','083F','083H','0908','0924','0928','0930','0953','0822','0811','0193','5711')
