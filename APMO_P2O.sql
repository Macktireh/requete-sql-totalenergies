...
/** Table KNVV => KNVV **/
FROM SAPC47.KNVV@P2O.WORLD KNVV,

/** Table KNVV - Replace the country => T2 **/
(SELECT ZZPRCTR, max(ERDAT) lastdate FROM SAPC47.KNVV@P2O.WORLD
WHERE substr(ZZPRCTR,1,2) in ('KH','CN','FJ','ID','PK','PG','PH','FJ', 'NC', 'PF')
--WHERE substr(ZZPRCTR,1,2) in ('MU')
AND VTWEG LIKE '20' GROUP BY ZZPRCTR) T2,

/** Table CEPC => CEPC **/
(SELECT PRCTR, KHINR, DATAB , MAX(DATBI) AS DATBI FROM SAPC47.CEPC@P2O.WORLD WHERE SUBSTR(PRCTR,6,1) = 'S' 
GROUP BY PRCTR, KHINR, DATAB) CEPC,

/** Table KNA1 => KNA1 **/
SAPC47.KNA1@P2O.WORLD KNA1,

/** Table KNA1 => T6 **/
(SELECT KUNNR, MAX(ERDAT) AS lastdate2
    FROM SAPC47.KNA1@P2O.WORLD
    GROUP BY KUNNR) T6,


/** Table CEPCT => CEPCT **/
SAPC47.CEPCT@P2O.WORLD CEPCT
...
AND KNVV.VKORG in ('0882','011F','0943','ZID1','0936','ZPG1','0116','ZFJ1', '0850', '0852')
